...
$down_cat_id = $_POST['down_cat_id'];

include "config.php";
    $que = "SELECT p.pro_id, p.pro_name, p.price, p.discount, p.pro_image FROM product p WHERE p.cat_id = {$cat_id} AND p.sub_cat_id = {$sub_cat_id} AND p.under_cat_id = {$under_cat_id} AND p.down_cat_id = {$down_cat_id} AND p.stock_quantity != 0;";
